...
class LabPackageTests(unittest.TestCase):
    ...
    desktop: the exe, the lab yaml, the PsychoPy launcher and a
    generated source/ tree, nothing else (Basil: no text files, keep
    it minimal). The exe, the yaml copy and source/ are build
    products (gitignored, regenerated by scripts/build_lab_package.py);
    anything present must match its source of truth, and a bare
    double-click of the frozen exe must find the sibling config on
    its own."""

    PKG = REPO / "docs" / "lab_package"
    TOP_LEVEL = ("Finger Rehab.exe", "eeg_lab.yaml",
                 "run_in_psychopy.py", "source")
    ...
    def test_psychopy_launcher_invokes_main_py_only(self) -> None:
        # The PsychoPy route must not become a second entry point:
        # same rule as the launchers. Its own file name is the one
        # other .py it may mention.
        text = (self.PKG / "run_in_psychopy.py").read_text()
        for match in re.findall(r"\S+\.py\b", text):
            base = match.replace("\\", "/").rsplit("/", 1)[-1]
            base = base.strip("\"'`(")
            if base == "run_in_psychopy.py":
                continue
            self.assertEqual(base, "main.py",
                             f"run_in_psychopy invokes {match}")
