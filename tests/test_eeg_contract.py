...
class WriterConfigTests(unittest.TestCase):
    ...
    def test_box_and_mode_reach_the_status_record(self) -> None:
        ...
        # 1.4 added Force Pilot's run-start and segment-edge codes (23,
        # 24) and the name field on every raw.csv eeg row. No existing
        # code changed meaning, which the band and uniqueness tests
        ...
        self.assertEqual(st["codes_version"], "1.4")
        from finger_rehab.hardware.eeg_trigger import CODES, MODE_IDS
    ...
    def test_metadata_carries_the_latency_and_the_offsets(self) -> None:
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                ...
                eeg = meta["eeg"]
                self.assertEqual(eeg["codes_version"], "1.4")
...
class LabPackageTests(unittest.TestCase):
    ...
    regenerated by scripts/build_lab_package.py); anything present
